...
void successparseStatusCase(QString instr, InstructionParser &parser){
    ParseResult result = parser.parseLine(TEST_LINE_NUMBER, instr);
...
void TestParser::RType_Parse_Fail(){
    ...
    parseStatusCase(
        "ADD R1, R2, 0xABCD",
        ParseStatus::fail("Expected register token for: r3 instead of: 0xABCD", {"0xABCD", 12, 6, TEST_LINE_NUMBER}),
        parser
        );
    parseStatusCase(
        "ADD R1, R2, R3, R4",
        ParseStatus::fail("Too many tokens for the format", {",", 14, 7, TEST_LINE_NUMBER}),
        parser
        );
    parseStatusCase(
        "ADD R1, R2",
    ...
    parseStatusCase(
        "ADD R1, R2(R3)",
        ParseStatus::fail("Expected separator token: , instead of: (", {"(", 10, 5, TEST_LINE_NUMBER}),
        parser
        );
}
...
void TestParser::IType_Parse_Fail(){
    ...
    parseStatusCase(
        "LDH R2, 0x0022, R1",
        ParseStatus::fail("Expected separator token: ( instead of: ,", {",", 14, 5, 15}),
        parser
        );
    parseStatusCase(
        "LDH R2, R1, 0x0022",
        ParseStatus::fail("Expected hex immediate token for: i instead of: R1", {"R1", 8, 4, 15}),
        parser
        );
    parseStatusCase(
        "LDH R2, 0x0022(R1), R3",
        ParseStatus::fail("Too many tokens for the format", {",", 18, 8, 15}),
        parser
        );
    parseStatusCase(
        "ADDI R1, R2, 0x0022",
        ParseStatus::fail("Expected hex immediate token for: i instead of: R2", {"R2", 9, 4, 15}),
        parser
        );
    parseStatusCase(
        "ADDI R1, 0x0022",
...
void TestParser::IType_Jumps(){
    ...
    parseStatusCase(
        "BRZ R1, label_gt_max",
        ParseStatus::fail("Jump to label: label_gt_max out of range for I-Type encoding", {"label_gt_max", 8, 4, 15}),
        parser
        );
    parseStatusCase(
        "BRZ R1, label_gt_min",
        ParseStatus::fail("Jump to label: label_gt_min out of range for I-Type encoding", {"label_gt_min", 8, 4, 15}),
        parser
        );
}
...
void TestParser::JType_Parse_Fail(){
    ...
    parseStatusCase(
        "JUMP loop, R1",
        ParseStatus::fail("Too many tokens for the format", {",", 9, 3, 15}),
        parser
        );
}
